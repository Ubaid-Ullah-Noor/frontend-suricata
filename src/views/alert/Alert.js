...
const Alert = () => {
  ...
  const fetchByRules = async (start_date = '', end_date = '') => {
    ...
    const response = await apiService.getHandler(
      `api/alerts_by_rule?start_date=${start_date}&end_date=${end_date}`,
      authHeader,
    )
    ...
    // Doughnut Chart Data
    const rules = response_data.map((item) => item.rule) // Get an array of rule values
    const counts = response_data.map((item) => item.count) // Get an array of count values

    setRules(rules) // ["2027757", "2610102"]
    setRulesCount(counts) // [8, 4]
  }

  const groupByCountry = (data) => {
    let result = _.groupBy(data, function (item) {
      return item.country
    })
    let counts = []
  ...
  const fetchAlerts = async (start_date = '', end_date = '') => {
    ...
    const response = await apiService.getHandler(
      `api/alerts?start_date=${start_date}&end_date=${end_date}`,
      authHeader,
    )
    const response_data = await responseHandler(response)
    if (response_data) {
  ...
  const fetchLogs = async (page = 0, limit = perPage, start_date = '', end_date = '') => {
    ...
    const response = await apiService.getHandler(
      `api/all_alerts?offset=${page}&limit=${limit}&start_date=${start_date}&end_date=${end_date}`,
      authHeader,
    )
    const response_data = await responseHandler(response)
    if (response_data) {
            ...
            <CCardBody>
              <CRow>
                ...
                <CCol sm={12}>
                  <DataTable
                    columns={[
                      { name: 'Rule', selector: (row) => row.country, sortable: true },
            ...
            <CCardBody>
              <CRow>
                ...
                <CCol sm={12}>
                  <DataTable
                    columns={[
                      { name: 'Rule', selector: (row) => row.rule, sortable: true },
                      { name: 'Count', selector: (row) => row.count, sortable: true },
                    ]}
                    data={ruleData}
                    progressPending={loading}
